...
(async () => {
  ...
  const fileCountSummary = `${parsedResult.processedFileCount} file${
    parsedResult.processedFileCount === 1 ? "" : "s"
  }`;
  const processingErrorSummary = `${
    parsedResult.processingErrorCount
  } processing error${parsedResult.processingErrorCount === 1 ? "" : "s"}`;

  console.log("Done.\nSummary:");
  console.log(
    `\t${parsedResult.details.length} external SFTP server${
      parsedResult.details.length === 1 ? "" : "s"
    } polled`
  );
